...
impl AppleAccount {
    ...
    pub async fn get_app_token(&self, app_name: &str) -> Result<AppToken, Error> {
        ...
        // Vrať surovou GSA odpověď + session key `sk`, ať volající dešifruje
        // app token (`et`) GCM formátem "XYZ" + iv + ct + tag.
        let mut out = res;
        out.insert("sk".to_string(), plist::Value::Data(sk.to_vec()));
        Ok(AppToken {
            app_tokens: out,
